...
- `workflows` — markdown-formatted guidance for each of the four skill workflows.
- `tags` — canonical namespaces and notes on the tag system.

## How to compose the query workflow

When the user asks "find plays about X" or "what do we know about X?", prefer free-text search over tag enumeration.
...
- Multiple sources agree on the play but the user wants to see one specific case in detail.
...
When the user wants to add a new play, compose tag discovery, schema discovery, and the stdin-based `add`:
