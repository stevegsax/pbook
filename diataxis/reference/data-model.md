...
topic = "data-model"
covers = ["PlaybookEntry model with all fields (including embedding and feedback counters)", "FeedbackInput model", "PushExperienceInput model", "RetrievalInput and RetrievalResult models", "EntryType enum values", "Database schema (entries table with embedding and feedback counters)", "Ingested sessions tracking table (ingested_sessions)", "SessionInfo model for transcript discovery"]
...
For the ingestion workflow, see [How to Ingest Transcripts](/howto/ingest-transcripts/).